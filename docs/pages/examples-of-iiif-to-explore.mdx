...
If you don't have some specific IIIF content that you wish to view, the following is a short list of a few examples from various institutes in the UK:
...
## Importing an existing Manifest into your Sandbox
...
![Import existing Manifest](https://digirati-tanc-workshops.pages.dev/import-manifest-step-zero.png)

Select the option to Import, which should import the valid IIIF Manifest, displaying the Manifest in the Portal view. You need to add a 'slug' which acts as a unique identifier for the Manifest you are importing.

![Add the slug for import](https://digirati-tanc-workshops.pages.dev/import-manifest-step-one.PNG)

When successfully imported, your copy of the Manifest will be presented in the Portal Manifest view where you can browse the descriptive metadata and view the canvas(es) contained in the Manifest. Note that the assets - images, audio, video etc. that form part of the manifest will not be imported, the manifest simply points to the existing content.

![View existing Manifest in Portal view](https://digirati-tanc-workshops.pages.dev/import-manifest-step-two.PNG)
...
![Edit existing Manifest](https://digirati-tanc-workshops.pages.dev/import-manifest-step-three.PNG)

We will discuss aspects of the metadata included in IIIF Manifests, and if anyone has questions we'll try to address these.
